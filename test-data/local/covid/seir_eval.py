...
@click.option(
    "--contacts_matrix_file",
    "-cm",
    type=click.Path(exists=True),
    help="Path to contact matrix file",
)
@click.option("--visualize-compartments", "-vc", type=bool, is_flag=True)
@click.option(
    "--output_file_format",
    "-of",
...
@click.option(
    "--output_file", "-o", default="out.csv", type=str, help="Path to output file."
)
def main(
    config_file,
    contacts_matrix_file,
    ...
    def export_column(column_name, file_name):
        hospitalized = results[column_name].fillna(0).values.tolist()
        with open(file_name, "w") as f:
            f.write("\n".join(map(convert_num, hospitalized)))

    export_column("infected (active)", "infected_0.out")
    export_column("hospitalized (active)", "hospitalized_0.out")
    export_column("in ICU", "intensive_0.out")
    export_column("removed", "dead_0.out")
